...
import loss as L
import pandas as pd
from data import data_loader_semi, o_data_semi, g_data_cell_semi, inf_data_loader, o_data
...
def train_semi(opt,semi_iter): 
    ...
    gpath_cell2 = opt.saveroot + "/"+str(semi_iter)+"_cell/"
    opath2 = opt.saveroot + "/b_scan_img/"
    train_data_LD, valid_data_LD = data_loader_semi(opath, opath2, fold)
    ...
    for EPOCH in range(train_epoch):
        ...
        for _, t_batch_num in enumerate(train_data_LD):
            ...
            img_sub  = o_data_semi(opath, t_batch_num, width, height)
            t_gim_sub, t_gim_sub2 = g_data_cell_semi(gpath_cell, t_batch_num, width, height) 
        ...
        for _, v_batch_num in enumerate(valid_data_LD):
            ...
            val_sub  =  o_data_semi(opath, v_batch_num, width, height)
            v_gim_sub, v_gim_sub2 = g_data_cell_semi(gpath_cell, v_batch_num,width, height)
    ...
    # save last epoch
    torch.save(model.state_dict() ,'{}_{}.pkl'.format(opt.name,semi_iter))
    print("----saving successfully with name: {}_{}.pkl----".format(opt.name,semi_iter))
    ...
    df.to_csv(opt.name+"2D_"+str(semi_iter)+".csv",index=False)
    return 

# Vertical Direction
def test_v_semi(opt,semi_iter):
    ...
    for _, t_batch_num in enumerate(test_data_LD):
        ...
        for idx , train_filename in enumerate(sorted(t_batch_num)):
            ...
            if(i%40==0):
                ...
                b_scan_pgt_1.save(opt.saveroot+"/"+str(semi_iter+1)+"_cell/"+"h1_"+train_filename[:21]+".png")
                b_scan_pgt_2.save(opt.saveroot+"/"+str(semi_iter+1)+"_cell/"+"h2_"+train_filename[:21]+".png")
    return 

# Horizontal Direction
def test_h_semi(opt,semi_iter):
    ...
    for _, t_batch_num in enumerate(test_data_LD):
        ...
        for idx , train_filename in enumerate(sorted(t_batch_num)):
            ...
            if(i%40==0):
                b_scan_pgt_1 = Image.fromarray(np.uint8(b_scan_pgt[:,:500]*255))
                b_scan_pgt_2 = Image.fromarray(np.uint8(b_scan_pgt[:,500:]*255))
                b_scan_pgt_1.save(opt.saveroot+"/"+str(semi_iter+1)+"_cell/"+"v1_"+train_filename[:21]+".png")
                b_scan_pgt_2.save(opt.saveroot+"/"+str(semi_iter+1)+"_cell/"+train_filename[:21]+".png")
    return 